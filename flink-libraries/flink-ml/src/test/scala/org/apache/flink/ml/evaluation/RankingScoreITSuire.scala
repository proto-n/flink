...
import org.apache.flink.ml.pipeline.{PredictDataSetOperation, RankingFromRatingPredictOperation, RankingPredictor, TrainingRatingsProvider}
...
class RankingScoreITSuire extends FlatSpec with Matchers with FlinkTestBase {
  ...
  it should "join recommendations with test correctly" in {
    ...
    val joined = RankingScores.PrecisionAndRecallScoresUtilities.joinWithTest(recommendations, test).collect()
  ...
  it should "calculate precisions correctly" in {
    ...
    val precisions = RankingScores.PrecisionAndRecallScoresUtilities.precisions(recommendations, test).collect()
  ...
  it should "calculate recalls correctly" in {
    ...
    val recalls = RankingScores.PrecisionAndRecallScoresUtilities.recalls(recommendations, test).collect()
  ...
  it should "count the types per user up to k correctly" in {
    ...
    val counted = RankingScores.PrecisionAndRecallScoresUtilities.countTypesUpToK(setToCount).collect()
    counted.toSet shouldEqual Set(
      (1,1,1,0,2),
      (1,2,1,1,2),
      (1,3,2,1,1),
  ...
  it should "be possible to evaluate a RankingPredictor using a scorer" in {
    ...
    implicit val rankingFromRatingPredictOperation = new RankingFromRatingPredictOperation(mockedPredictor)
    ...
    // todo: implicit scoping
    val result = scorer.evaluate(test, predictor, new ParameterMap)(Scorer.defaultRankingEvalPrepare)
